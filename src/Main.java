...
public class Main {
    ...
    public static void printTuesdays(List<Entry> entries) {
        ...
        System.out.println("For Loop:");
        // write for loop
        for (Entry tuesdayEntries: entries ) {
            if (tuesdayEntries.getDay() == Day.TUESDAY) {
                System.out.println(tuesdayEntries);
            }
        }
    ...
    public static void countTueWedThur(List<Entry> entries) {
        ...
        int count = 0;
        // write for loop
        for (Entry midDayEntry: entries) {
            if (midDayEntry.getDay() == Day.TUESDAY || midDayEntry.getDay() ==  Day.WEDNESDAY || midDayEntry.getDay() == Day.THURSDAY){
                count += 1;
            }
        }
